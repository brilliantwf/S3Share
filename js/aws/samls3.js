...
function showS3BucketContents() {
    ...
    if (prefix === username || prefix === 'shared') {
...
function sendEmailAuthenticate(to) {
    ...
    for (index = 0; index < x.length; ++index) {
        ...
        sendFile += "- <a class=\"ulink\" href=\"https://ec2-52-80-174-239.cn-north-1.compute.amazonaws.com.cn/authenticated.html?auth=true&bucketname=" + bucket + "&key=" + file + "&user=" + to + "&hash=" + hashInBase64 + "\" target=\"_blank\">" + file + "</a><br>"
    }
    AWS.config.region = awsConfig.regionName;
...
function sendEmailUnauthenticate(to) {
    ...
    for (index = 0; index < x.length; ++index) {
        file = document.getElementById(x[index]).value;
        var hashInBase64 = createHmac(to, file);
        sendFile += "- <a class=\"ulink\" href=\"https://ec2-52-80-174-239.cn-north-1.compute.amazonaws.com.cn/authenticated.html?auth=false&bucketname=" + bucket + "&key=" + file + "&user=" + to + "&hash=" + hashInBase64 + "\" target=\"_blank\">" + file + "</a><br>"
    }
    var ses = new AWS.SES({
        params: {
            Destination: {
